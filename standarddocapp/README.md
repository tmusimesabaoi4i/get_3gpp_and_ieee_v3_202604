...
大きな画像が HTML のレイアウトを崩さないよう、次の 2 段構えでサイズを整えます。

1. **元の Word の表示サイズを反映**: 文書 (`word/document.xml`) に記録された各
   画像の表示寸法 (`wp:extent` / VML の `width`) を読み取り、HTML 側の画像幅に
   設定します。これにより「大きく貼った図は大きく、小さいアイコンは小さく」、
   原文書に近い見た目になります（画像数が一致したときのみ適用する安全方式）。
2. **CSS による上限**: いずれの画像も `max-width: 100%`（列幅を超えない）・
   `height: auto`（縦横比維持）・`max-height: 85vh`（極端に縦長でも画面に収まる）
   で制限し、100% 表示でもレイアウトが崩れないようにします。
